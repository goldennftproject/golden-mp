...
/* 20/9: este test mide la ENTREGA de la expansión, no la obra (ver test-expansion-obra.js) */
function obraApagada(g){ g.GF.EXP_OBRA = 0; return g; }
let fallos=0;
const ok=(n,c,d)=>{if(!c)fallos++;console.log((c?"  ok   ":"  FALLA")+"  "+n+(d?"   "+d:""));};

// 1) reinicio normal: vuelve EXACTAMENTE a donde estabas
{
  const g=obraApagada(juego(1.6,120,340));
...
{
  const g=obraApagada(juego(1.3,0,0));
...
{
  const g=obraApagada(juego(1,0,0)); g.G.level=99; g.G.expansiones=0;
