...
const G = ctx.G;
const CROP_ORDER = vm.runInContext("CROP_ORDER", ctx), PEZ_ORDER = vm.runInContext("PEZ_ORDER", ctx),
...
console.log("\nEL ÁLBUM ENTERO: SEIS FAMILIAS Y TODAS LAS PIEZAS DEL JUEGO");
{
  ...
  /* 27/8 (Pesca v4): la familia de peces es el catálogo nuevo y nada más. Las cuatro rarezas de
     la v2 y las nueve especies de la v3 se jubilaron con su sistema; los peces que un jugador
     tuviera se le vendieron al precio que tenían (ver mudanzaPescaV4). */
  ok("peces: las " + PEZ_ORDER.length + " especies de la v4",
    fam("peces").total === PEZ_ORDER.length, fam("peces").total + " láminas");
  ...
  ok("y el total es la suma de las seis familias",
    p.total === CROP_ORDER.length + PEZ_ORDER.length + RECIPE_ORDER.length + ORE_ORDER.length + ANIMAL_ORDER.length + MONSTER_ORDER.length,
    String(p.total));
...
console.log("\nLA PRIMERA VEZ REVELA — Y LAS DEMÁS NO CAMBIAN NADA");
{
  ...
  ctx.statAdd("pescar", "pez_globo", 1);   // 27/8: el catálogo de la v4; el « épico » de la v2 se jubiló
  ok("pescar un pez globo revela su lámina", fam("peces").piezas.find(p => p.k === "pez_globo").visto);
