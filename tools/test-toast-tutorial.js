...
console.log("\nEL CÓDIGO SE CONECTA A LOS EVENTOS REALES");
{
  ...
  const syncLayouts = (vivo.match(/const syncLayouts = \(\) => \{[^}]*\};/) || [""])[0];
  ok("resize recalcula guía, hotbar y toast",
    /syncRegistroPrompt\(\)/.test(syncLayouts) && /placeToast\(\)/.test(syncLayouts) && /addEventListener\("resize", syncLayouts\)/.test(vivo));
}
