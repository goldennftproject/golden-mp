...
function pescaV4Paso(dt, hold) {
  if (!P4) return;
  P4.t += dt;
  /* 31/8 — EL CARRETE ENTRE EL PIQUE Y LA MANO (today.docx: « agregar el mini juego »).
     Cuando el corcho se hunde ya no se entrega el pez directo: arranca la pelea de la barra
     vertical. El pez sigue decidido desde que se pagó (el carrete no re-sortea nada); acá solo
     se decide si se cobra YA o si queda pendiente para el próximo tiro. */
  if (!P4.carrete) {
    if (P4.t >= P4.dur) {
      const nv = (typeof nivelOficio === "function") ? nivelOficio("fishing") : 1;
      P4.carrete = carreteNuevo(P4.r.banda, nv);
      const sc = pescaEscena();
      if (sc && sc.pescaPanel) sc.pescaPanel(true);
      if (sc && sc.pescaBurbujas) sc.pescaBurbujas();
    }
    return;
  }
...
/* 31/8, la ida y la vuelta en el mismo día — y vale la pena dejar el porqué de las dos:
   · IDA: Suren dijo « las escamas se obtienen en el tablero » y esto se mudó al tablón del
     pueblo, bajo la lógica de que dos tableros de pedidos es uno de más.
   · VUELTA: dirección lo vio andando y decidió lo contrario — « en el tablón de pesca quedan
     mejores… que sea solo de pesca y en el tablero del pueblo solo se pidan cosechas, maderas
     y minerales ». O sea: no un tablón único, sino CADA TABLERO CON SU OFICIO. La Lonja es el
     tablero de la pesca; el del pueblo, el de la granja — y dejó de pedir peces (pedPool). */
  ...
  if (G.armasUnlocked) {   // doc maestro 2/8: 4 tipos × 5 rarezas con daño aleatorio y buff
    ...
    ARM_TIPOS.forEach(tipo => {
      ...
      ARM_RAREZAS.forEach(rar => {
        ...
        armas += '<div class="forge-row ' + (eqNow ? "eq" : "") + '"><div class="fic"><img src="' + GF.spr(w.sprite || td.sprite) + '"></div><div class="finfo"><div class="fnm">' + w.label + plusTag + (own ? ' <span class="tag">' + G.weapons[id].dur + '/' + w.dur + '</span>' : '') + '</div><div class="fds">Daño ' + w.min + '–' + w.max + ' · ' + (w.buffVal ? td.buffLabel + ' ' + w.buffVal + (tipo === "arco" ? "/s" : "%") + ' ' + BUFF_DESC[tipo] : 'sin bono — en madera es un palo') + '</div><div class="fds">' + (own ? 'Reparación en la pestaña Reparar' : 'Costo: ' + cs) + '</div></div><div class="fbtns">' + btns + '</div></div>';
      });
    });
  }
...
/* (pdSeccionPesca vivió acá unas horas del 31/8: los pedidos de pesca colgados de este tablón.
   Dirección lo vio y mandó lo contrario — cada tablero con su oficio: la pesca en la Lonja,
   y este tablón pidiendo solo lo de la granja. El porqué completo, sobre refreshLonja.) */
...
function refreshConfig() {
  ...
  /* 18/9 — CERRAR SESIÓN. El texto del aviso dice las dos cosas que el jugador necesita saber
     ...
     cómo entro? » en algo que ya está contestado. */
  if (bS) bS.onclick = async () => {
    if (bS.disabled) return;
    let email = "";
    try { const c = await cuentaEstado(); email = (c && c.email) || ""; } catch (e) {}
    askConfirm("Se va a guardar tu granja y después salís de la cuenta en este navegador." +
      (email ? " Para volver, entrás con " + email + " y el enlace del correo." : "") +
      " Tu granja no se borra.",
      async () => {
        bS.disabled = true;
        const r = await (typeof cerrarSesion === "function" ? cerrarSesion() : { error: "no disponible" });
        if (r && r.ok) { toast("Sesión cerrada"); setTimeout(() => location.reload(), 400); }
        else { bS.disabled = false; toast("No se pudo: " + ((r && r.error) || "error")); }
      }, { title: "Cerrar sesión", yes: "Cerrar sesión", yesClass: "red", no: "Cancelar", noClass: "ghost" });
  };
}
